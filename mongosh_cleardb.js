// Removes all documents for a single device across every non-system collection
// in the target database. The collections themselves and their indexes are
// preserved — this uses deleteMany({ device_name: ... }), NOT drop() and NOT an
// unfiltered deleteMany({}).
//
// Values are read from environment variables set by the wrapper shell script
// (mongosh_cleardb.sh), which sources them from mongosh_db.conf:
//   DEVICE_NAME  the device whose documents should be removed
